...
<script type="text/javascript">
    initAoc = function()
    {
        YAHOO.oxid.container1 = new YAHOO.oxid.aoc( 'container1',
                                                    [ [{ foreach from=$oxajax.container1 item=aItem key=iKey }]
                                                      ...
                                                      [{ /foreach }] ],
                                                    '[{ $oViewConf->getAjaxLink() }]cmpid=container1&container=category_rights_buyable&synchoxid=[{ $oxid }]'
                                                    );
        ...
        YAHOO.oxid.container2 = new YAHOO.oxid.aoc( 'container2',
                                                    [ [{ foreach from=$oxajax.container2 item=aItem key=iKey }]
                                                      ...
                                                      [{assign var="sSep" value=","}]
                                                      [{ /foreach }] ],
                                                    '[{ $oViewConf->getAjaxLink() }]cmpid=container2&container=category_rights_buyable&oxid=[{ $oxid }]'
                                                    );
        YAHOO.oxid.container1.modRequest = function( sRequest )
        {
            iRange = ($('range0').checked)?$('range0').value:$('range1').value;
            return sRequest+'&oxrrapplyrange='+iRange;
        }
        YAHOO.oxid.container1.getDropAction = function()
        {
            iRange = ($('range0').checked)?$('range0').value:$('range1').value;
            return 'fnc=addgrouptocatbuy&oxrrapplyrange='+iRange;
        }
        YAHOO.oxid.container2.modRequest = function( sRequest )
        {
            iRange = ($('range0').checked)?$('range0').value:$('range1').value;
            return sRequest+'&oxrrapplyrange='+iRange;
        }
        ...
        {
            iRange = ($('range0').checked)?$('range0').value:$('range1').value;
            return 'fnc=removegroupfromcatbuy&oxrrapplyrange='+iRange;
        }
    }
    ...
    <table width="100%">
        ...
        <tr class="edittext">
          <td colspan="2">
            <input type="radio" name="range" id="range0" value="0" checked [{$readonly}]> [{ oxmultilang ident="CATEGORY_RIGHTS_APPLYLEVEL3" }]<br />
            <input type="radio" name="range" id="range1" value="1" [{$readonly}]> [{ oxmultilang ident="CATEGORY_RIGHTS_APPLYLEVEL4" }]<br /><br />
          </td>
        </tr>
        <tr class="edittext">
            <td align="center"><b>[{ oxmultilang ident="CATEGORY_RIGHTS_ALLGROUPS" }]</b></td>
            <td align="center"><b>[{ oxmultilang ident="CATEGORY_RIGHTS_SETGROUPS_BUYABLE" }]</b></td>
        </tr>
        <tr>
